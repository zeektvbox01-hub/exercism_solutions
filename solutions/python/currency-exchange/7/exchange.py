...
def exchangeable_value(budget:float, exchange_rate: float, spread: int, denomination: int) -> int:
    ...
    Variables:
        adjusted_exchange_rate: The rate taken as an exchange fee.
        exchanged_amount: The amount of foreign currency received after an exchange.
        number_of_bills: The number of bills received from an exchange
    ...
    adjusted_exchange_rate: float = exchange_rate * (1 + (spread / 100))
    exchanged_amount: float = exchange_money(budget, adjusted_exchange_rate)
    number_of_bills: int = get_number_of_bills(exchanged_amount,denomination)
