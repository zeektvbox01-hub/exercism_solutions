def resistor_label(colours):
    ...
    if bands == 1:
        return "0 ohms"
    if bands == 4:
        ohms = colour_code(colours[0]) * 10
        ohms += colour_code(colours[1])
        ohms *= colour_code_powers(colours[2])
        ohms = int(ohms)
        tolerance = colour_code_resistance(colours[3])
        return final_string(ohms,tolerance)
    if bands == 5:
        ohms = colour_code(colours[0]) * 100
        ohms += colour_code(colours[1]) * 10
        ohms += colour_code(colours[2])
        ohms *= colour_code_powers(colours[3])
        ohms = int(ohms)
        tolerance = colour_code_resistance(colours[4])
        return final_string(ohms,tolerance)
    raise ValueError("We know that some of these do exist in real life. However,this exercise does not cover them, so this will raise a error for now")
